...
if (! function_exists('get_setting_email_template_path')) {
    ...
    {
        $tempPath = "$module/$templateKey.tpl";
        $locale = app()->getLocale();

        if($locale != 'el' && explode('/',request()->path())[0] != 'admin'){
            
            $locale = explode('_',$locale)[0];
            $tempPath = "$module/$locale/$templateKey.tpl";
        }
        $template = apply_filters('setting_email_template_path', $tempPath, $module, $templateKey);
...
if (! function_exists('get_setting_email_subject_key')) {
    function get_setting_email_subject_key(string $type, string $module, string $templateKey): string
    {
        ...
        if($locale != 'el' && explode('/',request()->path())[0] != 'admin'){
            ...
            $locale = explode('_',$locale)[0];
            $key = $type . '_' . $module . '_' . $locale . '_' . $templateKey . '_subject';
        }
