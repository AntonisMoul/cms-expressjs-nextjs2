...
class SearchService
{
    ...
    {
        // Ta prosthesa ego
        // An exoume business categories slugs, na gyrisoun se IDs (na ginoun mapping)
        if (isset($filters['business_categories'])) {
            $businessCategories = BusinessCategory::whereHas('slugable', function ($q) use ($filters) {
                $q->whereIn('key', $filters['business_categories']);
            })->pluck('id')->toArray();
            $filters['business_categories'] = $businessCategories;
        }
        // An exoume city slugs, na gyrisoun se IDs (na ginoun mapping)
        if (isset($filters['city'])) {
            $cities = City::whereHas('slugable', function ($q) use ($filters) {
                $q->whereIn('key', $filters['city']);
            })->pluck('id')->toArray();
            $filters['city'] = $cities;
        }
        ...
        if ($typeSelected) {
            // if (isset($filters['business_categories'])) {
                if (isset($filters['business_categories']) && !isset($this->adsQuery)) {
                $this->businessQuery = Business::searchByCategory($filters, 'business_categories');
                // Ta prosthesa ego - Add Ads support for business categories
                $this->adsQuery = Ads::query()
                    ->business($filters)
                    ->with(['business', 'business.categories', 'business.city']);
            }
        ...
        if (!$typeSelected && !$scopesSelected) {
            ...
            // Ta prosthesa ego - Add Ads support
            //$this->adsQuery = Ads::applyBasicFilters($filters)->with(['business', 'business.categories', 'business.city']);
        }
    ...
    {
        $searchScopes = $filters['searchScope'] ?? null;
        $user = Auth::guard('member')->user() ?? request()->user('sanctum');
        $wishlist =  $user->wishlist();
        if (isset($searchScopes)) {
            $wishlist = $wishlist->filter(function ($item) use ($searchScopes) {
                return in_array(class_basename($item), $searchScopes);
            });
        }
