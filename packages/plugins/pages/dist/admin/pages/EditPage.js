...
export function EditPage({ id }) {
    ...
    const fetchPage = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Load SEO data from meta
                if (pageData.meta) {
                    const seoMeta = pageData.meta.reduce((acc, meta) => {
    ...
    const handlePublish = async () => {
        try {
            const response = await fetch(`/api/v1/pages/${id}/publish`, {
                method: 'POST',
                credentials: 'include',
            });
            if (response.ok) {
    ...
    const handleUnpublish = async () => {
        try {
            const response = await fetch(`/api/v1/pages/${id}/unpublish`, {
                method: 'POST',
                credentials: 'include',
            });
            if (response.ok) {
