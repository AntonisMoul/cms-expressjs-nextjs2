...
export function MediaLibrary({}: MediaLibraryProps) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const promises = selectedFiles.map(fileId =>
        fetch(`/api/v1/media/files/${fileId}`, {
          method: 'DELETE',
          credentials: 'include',
        })
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600">
        <button
          ...
          className={`hover:text-blue-600 ${currentFolder === null ? 'text-blue-600 font-medium' : ''}`}
        >
          All Files
        </button>
        {currentFolder && (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {files.map((file) => (
                  ...
                  >
                    ...
                    <input
                      type="checkbox"
                      className="absolute top-2 left-2 w-4 h-4 text-blue-600 bg-white border-gray-300 rounded focus:ring-blue-500"
                        ...
                        <td className="px-6 py-4">
                          <div className="w-10 h-10 bg-gray-100 rounded flex items-center justify-center">
                            {file.mimeType.startsWith('image/') ? (
                              ...
                              <i className={`${getFileIcon(file.mimeType)} text-gray-600`}></i>
                            )}
                          </div>
                        </td>
                        <td className="px-6 py-4">
