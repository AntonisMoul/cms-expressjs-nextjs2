...
class MenuService {
    ...
    async updateMenuNode(id, data) {
        ...
        if (data.iconFont !== undefined)
            updateData.iconFont = data.iconFont;
        if (data.position !== undefined)
            updateData.position = data.position;
        if (data.title !== undefined)
            updateData.title = data.title;
        if (data.cssClass !== undefined)
            updateData.cssClass = data.cssClass;
    ...
    async reorderMenuNodes(menuId, nodeOrders) {
        // Update all nodes in a transaction
        await this.prisma.$transaction(nodeOrders.map(order => this.prisma.menuNode.update({
            where: { id: order.id },
            data: {
    ...
    async assignMenuToLocation(menuId, location) {
    ...
    async removeMenuFromLocation(location) {
        await this.prisma.menuLocation.deleteMany({
            where: { location },
        });
    }
    ...
    // Helper methods
    async updateParentHasChild(parentId) {
